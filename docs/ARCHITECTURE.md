...
connector 将该 key 原样映射为 Hermes `chatId`。同一节点重连或重启后仍命中同一会话，不同节点则在 Hermes 历史、单 session 执行锁和 quarantine 上完全分离。启动时会在同一个 SQLite `IMMEDIATE` 事务内读取并幂等重建旧数据库中的共享 session key，同时把旧隔离记录展开到受影响节点；事务快照与改写之间不会接纳新的 legacy session job。Hermes 旧共享历史不会迁移，因为无法判断其中每段上下文属于哪个节点，升级会有意切断这段历史连续性。
